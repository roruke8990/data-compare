...
function compareData() {
    ...
    const map2ByTitle = new Map();
    list2.forEach(item => map2ByTitle.set(item.filename, item));

    const checkedTitlesIn2 = new Set();

    list1.forEach(item1 => {
        const item2 = map2ByTitle.get(item1.filename);
        if (item2) {
            checkedTitlesIn2.add(item1.filename);
            if (item1.contentId === item2.contentId) {
                results.match.push(item1);
            } else {
                results.diff.push({ filename: item1.filename, side1: item1, side2: item2 });
            }
        } else {
    ...
    list2.forEach(item2 => {
        if (!checkedTitlesIn2.has(item2.filename)) {
            ...
            results.diff.forEach(group => {
                html += `
                    <tr style="background-color: #fafafa;">
                        <td rowspan="2" style="vertical-align: middle; border-bottom: 2px solid var(--border-color);"><strong>${group.filename}</strong></td>
